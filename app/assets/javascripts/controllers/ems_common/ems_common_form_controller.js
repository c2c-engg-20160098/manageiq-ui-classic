ManageIQ.angular.app.controller('emsCommonFormController', ['$http', '$scope', '$attrs', 'emsCommonFormId', 'miqService', '$timeout', 'API', function($http, $scope, $attrs, emsCommonFormId, miqService, $timeout, API) {
  var init = function() {
    $scope.emsCommonModel = {
      ...
      provider_region: '',
      // C2C: Added code for project_name
      project_name: '',
      default_userid: '',
      default_password: '',
      amqp_userid: '',
  ...
  $scope.isBasicInfoValid = function() {
    // C2C: Added condition/code for OTC cloud provider - otc
    if (($scope.currentTab === 'default' && $scope.emsCommonModel.emstype !== 'azure') &&
      ($scope.emsCommonModel.emstype === 'ec2' ||
       ['kubevirt', 'nuage_network', 'openstack', 'openstack_infra', 'rhevm', 'scvmm', 'vmwarews', 'vmware_cloud', 'otc'].includes($scope.emsCommonModel.emstype) &&
      ...
      ($scope.emsCommonModel.emstype === 'openstack' || $scope.emsCommonModel.emstype === 'openstack_infra' || $scope.emsCommonModel.emstype === 'otc')) &&
